...
export default function ProfileSetup({ nickname, email, phone, region, realName }: Props) {
  ...
  function validate(): boolean {
    ...
    const trimmedName = form.real_name.trim()
    if (trimmedName.length < 2 || trimmedName.length > 20) {
      newErrors.real_name = '실명은 2~20자로 입력해주세요'
    }
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="mb-2 block text-xs font-bold text-primary-700 tracking-tight">
              실명<span className="text-accent-500 ml-0.5">*</span>
            </label>
            <input
              type="text"
            ...
            />
            {errors.real_name && (
              <p className="mt-1 text-xs text-error">{errors.real_name}</p>
            )}
          </div>
