...
- `src/app/(next)/` — **전면개편 5탭 UI (토스 스킨)**: `/home` `/meet` `/talk` `/shelf` `/me`(+`/me/notifications` 알림함). `site_settings.next_ui` 플래그 뒤에 있어 OFF면 회원 노출 0. 레이아웃이 온보딩 게이트(`isOnboarded()` — 미완성자는 `/`로) 수행. 표현 컴포넌트는 `src/components/next/` (NextNav, TossUI, HomeView/MeetView/TalkView). 플래그 ON 시 `/`는 온보딩 완료자만 `/home`으로 리다이렉트.
  **상세·신청·결제도 `(next)` 안에 있다** (2026-09-19 실측) — `meet/[id]`(상세, `MeetingDetailView`) · `meet/[id]/apply`(신청 확인, `ApplyConfirmView`) · `meet/[id]/done`(신청 완료, `RegistrationDoneView`) 3장. 판정은 `loadMeetingDetail` 한 벌을 쓰고 버튼은 구 화면과 **같은 `MeetingActionButton`**(`skin="toss"`)이라 결제·환불 코드가 두 벌이 되지 않는다. 구경로는 역방향으로 넘어온다 — `(main)/meetings/[id]/page.tsx:42-43`이 `next_ui` ON이면 `/meet/[id]`로, `.../confirm/page.tsx:22-27`이 `/meet/[id]/done`으로 리다이렉트(알림톡 버튼 URL이 구경로라 재심사 없이 하위 호환).
  **환불 규정은 결제 전에 보인다** — `ApplyConfirmView.tsx:79`의 `<RefundNotice showHeading />`가 결제 버튼(`:81`) **위에** 렌더된다. 비율·날짜는 하드코딩이 아니라 `getRefundScheduleByType()` 계산값(`RefundNotice.tsx:22`) — 토론 7/3, 정기 3/2를 두 곳에 적지 않기 위해서다. 진입 경로도 강제돼 있다: 신청 가능하면 상세에 결제 버튼 대신 `/meet/[id]/apply` 링크만 뜬다(`MeetingDetailView.tsx:218-237`).
  ⚠️ **남은 구 화면 이탈 3곳** — `(next)` **page 파일**에서 `(main)`으로 나간다. `shelf/page.tsx:105` → `/my#library`(책 담기·관리) · `me/page.tsx:80` → `/my#registrations`(신청 내역) · `me/page.tsx:92` → `/my`(프로필·설정). 넘어가면 하단 탭이 5개→2개로 바뀐다(`BottomNav`는 `/`·`/my` 2탭). 서재 1건은 **버그가 아니라 예정된 작업**이다 — 「서재 탭으로 단일화」 결정(`DECISIONS.md` 2026-09-19, ⏸ 보류 · 착수는 켜는 날 이후). 나머지 2곳은 결정 기록이 없다(의도 불명). 반면 `src/components/next/**` 안에서 `(main)`으로 나가는 링크는 **0개**이고, 유일한 외부 링크는 `MeetingDetailView.tsx:205`의 `/admin/meetings/[id]`(운영자 전용, 의도된 이탈).
  🔴 **여기 있던 🔴 경고 2건(*"상세·신청·결제가 없다"* · *"회원이 환불 조건을 모른 채 돈을 보낸다"*)은 2026-09-19에 지웠다 — 둘 다 사실이 아니었다.** 왜 낡았는지가 중요하다: 고친 커밋 `06b3e00`(2026-08-25 17:58 *"환불 조건을 모른 채 돈을 보내던 자리에 화면을 세웠다"*)이 경고를 적은 커밋 `c8a8b89`(같은 날 22:44)의 **직전 커밋**이다. 문서가 나중에 낡은 게 아니라 **적히는 순간 이미 틀린 문장**이었다 — 같은 세션에서 코드를 고쳐놓고, 문서에는 고치기 **전**에 돌린 조사 결과를 그대로 옮겼다. **조사 문서를 CLAUDE.md로 옮길 때는 그 사이에 들어간 커밋을 먼저 본다.** 당시 조사 원본(현행 사실로 읽지 말 것 — 2026-08-25 스냅샷): `docs/agent-team/조사/2026-08-25-개편-빠진화면-코드측.md` · `docs/superpowers/mockups/2026-08-25-개편-시안대조/대조표.md`
...
- **Admin 메뉴 단일 소스** (Phase 3 M7 Step 2): `src/components/admin/adminMenu.ts`가 **10개** 메뉴를 3개 그룹(운영 5 / 콘텐츠 3 / 시스템 2)으로 정의. `adminOnly: true`(= editor에서 숨김)는 **정산 · 공지 보내기 · 알림톡 이력 · 설정 4개**다 — **배너·한 줄은 editor도 쓸 수 있다.** (2026-08-21 실측 정정. 이전에 "7개 메뉴 / 정산·배너·설정"이라 적혀 있었는데 개수도 목록도 틀렸고, 루트 `CLAUDE.md`의 역할 매트릭스와도 어긋났다. 코드가 루트 쪽 손을 들어준다.) `PlaceholderPage` 컴포넌트가 M8/M10 준비 중 라우트를 렌더. 새 admin 메뉴 추가 시 `adminMenu.ts`에만 항목 추가하면 AdminSidebar/AdminMobileNav 양쪽에 자동 반영
