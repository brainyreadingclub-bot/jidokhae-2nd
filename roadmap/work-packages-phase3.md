...
> 🔴 **2026-08-18 상태 공지 — M8·M9는 전면개편으로 대체되었다.**
> 회원 홈 리뉴얼과 콘텐츠 노출(배너·한 줄)은 이 문서의 계획 대신 **전면개편(5탭 × 토스 스킨 + 발제 스레드)**이
> 현행 기준이다 — 근거: `docs/superpowers/specs/2026-08-17-전면개편-토스5탭-design.md` + `검토문서/DECISIONS.md`(2026-08-15~18).
> M7은 완료, M10(정산)은 완료, M11(디자인 토큰)은 토스 스킨 결정과 정합 재검토 필요, M12(통합 검증)는 유효.
> 어긋남 전수와 정비 방침: `검토문서/2026-08-18-문서정비-계획.md`
