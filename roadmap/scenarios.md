...
### Scenario 4-1-09: 결제 성공 후 API Route 호출 실패 (네트워크 오류)

- **Given:** 사용자가 PG사 결제를 완료하여 payment_id를 받았다
- **When:** 프론트엔드가 `POST /api/registrations/confirm`을 호출했으나 네트워크 오류로 응답을 받지 못한다
- **Then:** 프론트에 "결제는 완료되었으나 신청 처리 중 문제가 발생했습니다. 잠시 후 내 신청 내역을 확인해주세요." 안내 메시지가 표시된다. registrations에 레코드가 생성되지 않는다 (MVP 한계 — 포트원 콘솔에서 운영자 수동 확인 필요)
- **선행 Scenario:** 4-1-01
