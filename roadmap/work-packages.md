...
**검증 기준:**
- [x] 테스트 결제 → 토스페이먼츠 승인 → registrations INSERT 확인 (status: confirmed)
- [x] 결제 금액 ≠ 모임비 → 거부 + 환불 확인
- [x] 정원 초과 시 자동 환불 + "마감" 응답 확인
- [x] DB Function 동시 호출 시 정원 이상 INSERT 불가 확인
...
**검증 기준:**
- [x] 목록 → 상세 → 신청하기: 결제 화면 진입까지 정확히 3클릭
- [x] 결제 완료 → 확정 화면 표시 → 목록 복귀 시 "신청완료" 뱃지
- [x] 결제 실패 → 상세 페이지 + 에러 토스트 → 바로 재시도 가능
- [x] 모바일에서 결제창이 정상 표시됨 (redirect 방식)
...
**검증 기준:**
- [x] 내 신청 탭에서 신청한 모임 확인 가능
- [x] 상태 뱃지가 조건에 따라 정확히 표시됨
- [x] 운영자: 모임 상세에서 신청자 이름 + 결제 상태 확인 가능
- [x] 내역에서 모임 터치 → 상세 페이지 이동 → 올바른 버튼 표시
...
- [x] 3일 전 취소 → 100% 환불 확인
- [x] 2일 전 취소 → 50% 환불 확인
- [x] 전날/당일 취소 → 0원 환불, 취소는 완료됨
- [x] 모임 다음 날 → 취소 버튼 자체가 미표시
- [x] 취소 후 모임 상세 → "신청하기" 버튼 다시 표시 (자리 있을 때)
- [x] 취소 → 재신청 → 새 registrations 레코드 생성 확인
- [x] 내 신청 내역에서 "취소됨" 뱃지 표시
...
**검증 기준:**
- [x] 신청자 3명인 모임 삭제 → 전원 100% 환불 + deleted 상태
- [x] 환불 1건 실패 시뮬레이션 → deleting 유지 + "환불 미처리" 뱃지 (코드 검증)
- [x] 재시도 → 실패 건만 환불 시도 → 성공 시 deleted (코드 검증)
- [x] 삭제된 모임이 회원 목록에서 사라짐
- [x] deleting 모임은 운영자 목록에만 표시됨
...
**검증 기준:**
- [x] 10개 시나리오 전체 통과 (STEP 1~11, 2026-03-19)
- [x] 발견된 버그 전부 수정
- [x] 카카오톡 인앱 브라우저에서 전체 흐름 정상 동작 (STEP 11)
...
- [x] 회원이 카카오 로그인으로 접속할 수 있다
- [x] 모임 일정 목록이 보인다 (마감 상태 포함)
- [x] 모임을 선택하고 결제까지 완료하면 신청이 확정된다
- [x] 내 신청 내역을 확인할 수 있다
- [x] 회원이 직접 취소할 수 있고, 환불 규정에 따라 자동 환불된다
- [x] 운영자가 모임을 생성/수정/삭제할 수 있다
- [x] 운영자가 신청자 목록과 결제/취소 상태를 확인할 수 있다
...
| v1.5 | 2026-03-19 | M4/M5 검증 기준 전체 체크. WP6-1 검증 완료. WP6-2 MVP 출시 체크리스트 체크 (WP6-1에서 검증됨) |
