...
[포트원] ──Webhook──→ [API Routes: /api/webhooks/portone]
                       (결제 완료 백업 통지 — 프론트 콜백 실패 시 자동 복구)
...
위 ②에서 프론트엔드 콜백/redirect 복귀가 모두 실패한 경우 (인앱 브라우저 전환, 네트워크 끊김 등), 포트원이 서버 Webhook 엔드포인트(`/api/webhooks/portone`)에 직접 결제 완료를 통지한다. Webhook 핸들러는 서명 검증 → payment_id 기등록 확인 → 미등록 시 결제 검증 + 등록 처리. 이미 등록된 건은 무시한다.
