...
| 결제 (PG사) | 토스페이먼츠 | REST API + @tosspayments/payment-sdk | 직접 연동, 카드 결제 수수료 약 3.2~3.5% |
...
- **Next.js API Routes ↔ 토스페이먼츠**: 결제 승인(confirm), 환불 처리는 반드시 서버(API Routes)에서 토스페이먼츠 REST API를 호출
- **Next.js API Routes ↔ Supabase**: 결제 승인 후 신청 데이터 확정, 취소/환불 상태 업데이트
- **프론트엔드 ↔ 토스페이먼츠 SDK**: 결제창을 여는 역할만 (결제 승인은 서버에서)
...
[Next.js 프론트엔드 — Vercel]
   ...
   └── 민감한 처리 ──→ [Next.js API Routes — Vercel Serverless]
                           │              │              │
                           ▼              ▼              ▼
                     [Supabase DB]  [토스페이먼츠 REST API]  [카카오 OAuth]
                                     (결제승인/환불)

[토스페이먼츠] ──Webhook──→ [API Routes: /api/webhooks/tosspayments]
                            (결제 완료 백업 통지 — 프론트 redirect 실패 시 자동 복구)
...
| 모임 삭제 | 프론트 → API Route → (토스페이먼츠) → Supabase | 신청자 유무 관계없이 항상 API Route 경유. 신청자 있으면 환불 후 삭제, 없으면 바로 삭제 |
| 모임 신청 + 결제 | 프론트 → API Route → 토스페이먼츠 → Supabase | 결제 승인 필수, 정원 체크 필수 |
| 취소 + 환불 | 프론트 → API Route → 토스페이먼츠 → Supabase | 환불 API 호출, 금액 계산 |
...
- 결제 후 클라이언트 redirect 복귀 실패 (인앱 브라우저 전환, 네트워크 끊김) → 토스페이먼츠 Webhook 백업 경로로 자동 복구. Webhook도 실패 시 토스페이먼츠 관리자 콘솔에서 운영자 수동 확인
...
→ "신청하기" 버튼 (2클릭)
→ 토스페이먼츠 결제창 진입 (3클릭, redirect)
→ 결제 완료 → 서버가 POST /v1/payments/confirm 호출하여 승인
...
├ payment_id (토스페이먼츠 결제 paymentKey)
...
```
① 회원이 "신청하기" 클릭
→ [프론트엔드] 토스페이먼츠 SDK 호출 → 토스페이먼츠 결제창으로 redirect

② 회원이 결제 완료
→ 토스페이먼츠가 successUrl로 리다이렉트 (paymentKey, orderId, amount 파라미터 포함)

③ 복귀 페이지에서 API Route 호출 ("이 결제를 승인해줘")
→ [API Route] paymentKey로 기등록 여부 선행 체크 (멱등성 — 이미 등록됐으면 성공 반환)
→ [API Route] 미등록 시 토스페이먼츠 REST API POST /v1/payments/confirm 호출 (결제 승인)
→ [토스페이먼츠] 결제 승인 응답 (금액, 상태 확인)

④ API Route가 승인 완료 후 정원 체크
...
→ 정원 OK:
   ...
   → 프론트에 성공 응답
→ 정원 초과:
   토스페이먼츠 환불 API 호출 (전액 환불)
   → 프론트에 "마감" 응답
→ 이미 등록됨 (already_registered):
   환불 호출 후 프론트에 "마감" 응답
```

> **핵심 규칙**: 프론트엔드의 "결제됐다"는 말을 그대로 믿지 않는다. 서버가 토스페이먼츠에 직접 결제 승인(confirm)을 한다.
> **동시 신청 처리**: 정원 체크는 DB 트랜잭션 레벨에서 원자적으로 수행한다. 두 명이 동시에 마지막 1자리를 신청하면, 먼저 INSERT된 건만 성공하고 나머지는 환불 처리된다.

**백업 경로: 토스페이먼츠 Webhook**

위 ②에서 프론트엔드 redirect 복귀가 실패한 경우 (인앱 브라우저 전환, 네트워크 끊김 등), 토스페이먼츠가 서버 Webhook 엔드포인트(`/api/webhooks/tosspayments`)에 직접 결제 완료를 통지한다. Webhook 핸들러는 서명 검증 → paymentKey 기등록 확인 → 미등록 시 결제 승인 + 등록 처리. 이미 등록된 건은 무시한다.
...
1.5. SELECT COUNT(*) FROM registrations
     ...
     → count > 0이면 'already_registered' 반환 (동일 사용자 중복 등록 방어)
...
> **정원 초과 환불 실패 시**: 환불 API 호출이 실패하면, 에러를 로그에 기록하고 프론트에 "일시적 오류가 발생했습니다. 잠시 후 다시 시도해주세요." 안내 메시지를 반환한다. 결제 건은 토스페이먼츠 관리자 콘솔에서 운영자가 수동 확인 후 처리한다.
...
→ [API Route] 토스페이먼츠 환불 API 호출 (전액 또는 부분 환불, 0원이면 호출 생략)
...
→ 각 건: 토스페이먼츠 환불 API 호출 (100% 환불)

> **병렬 처리 필수**: Vercel Hobby 플랜의 Serverless Function 실행 시간 제한은 10초다. 토스페이먼츠 API 응답이 건당 1~3초이므로, 14명을 순차 처리하면 타임아웃된다. Promise.allSettled로 병렬 호출하면 전원 처리가 3초 내에 완료되고, 개별 실패도 격리된다.
→ 성공 시:
...
| 결제 승인은 서버에서 | 프론트엔드가 아닌 API Route에서 토스페이먼츠에 confirm 호출 |
...
| v1.6 | 2026-03-05 | MVP 검토 반영 5건: 시스템 다이어그램 Webhook 경로 추가, 위험성 문구 재작성, 결제 흐름 popup/redirect 이중 경로 + Webhook 백업 분리, DB Function 중복 등록 방어(1.5단계), 취소 환불 실패 분기 추가 |
| v1.7 | 2026-03-12 | 결제 연동 변경: 포트원 경유 → 토스페이먼츠 직접 연동. redirect only 결제 흐름, 서버 confirm API, Webhook 엔드포인트 변경 |
