...
### ✅ 켜는 날 게이트 — **절차는 전부 닫혔다.** 남은 것은 대표님 결정 2건뿐 (2026-09-10 재판정)

> ✅ **2026-09-10에 마지막 2건을 실제로 실행했다.** 아래 표 1번·3번은 18일간 *"결정은 났고 명령은 안 돌았다"* 상태였고, 오늘 관리자가 `gh`로 리모트를 바꿨다. **`gh pr view`로 사후 확인까지 마쳤다** — 결정 기록이 아니라 리모트 상태가 바뀐 것이다.
>
> 🔴 **retarget이 새 사실을 하나 만들었다 — 머지 순서가 이제 안전장치가 아니라 필수다.** `#53`의 base를 `main`으로 옮기자 GitHub이 `#53`을 **`#49`의 파일까지 포함한 덩어리로** 표시하기 시작했다(실측: `book-ask` 파일 4개가 `#53` 변경 목록에 함께 뜬다). `#53`이 `#49` 브랜치 위에 얹혀 있어서 생기는 정상 현상이고, **`#49`를 먼저 머지하면 `#53`은 다시 환영 알림톡만 남는다.**
> **`#49` → `#53` 순서를 어기면** 환영 알림톡과 물어보기 크론이 **한 커밋에 묶여 들어가** 되돌리는 단위가 다시 하나로 합쳐진다 — retarget으로 얻으려던 것을 그대로 잃는다. 런북 §1의 `#50 → #49 → #53`을 **순서 권고가 아니라 제약으로 읽을 것.**

| # | 남은 것 | 상태 |
|:--:|---|---|
| — | **예고 문구 3종 확정** (예고 띠 · 카톡 채널 공지 · 알림함 첫 공지) | 🙋 **대표님 대기** — 초안은 런북 §4에 있다. 예고 띠 기능은 **현행 `(main)` 홈에 이미 살아 있어** 플래그·배포 없이 문구 입력만으로 노출된다(`(main)/page.tsx:24`, `origin/main` 실측) |
| — | **켜는 날 날짜** | 🙋 **대표님 대기** — 정해져야 예고 문구의 "N월 N일"이 채워진다. 예고는 **켜기 1~2일 전** 게시(런북 §0) |

<details>
<summary>닫힌 절차 3건 (기록 — 다시 열지 말 것)</summary>

### ~~🔴 켜는 날 게이트 — 사전조건은 충족, 절차 4건이 남았다~~ (2026-08-21 재판정, 2026-09-10 종료)
...
| # | 남은 것 | 빠뜨리면 |
|:--:|---|---|
| 1 | ~~**PR #53의 base가 `main`이 아니다**(`feat/book-ask-cron`)~~ → ✅ **닫혔다** (2026-09-10, `gh pr edit 53 --base main` 실행 + `gh pr view` 확인. 현재 `base=main` · `MERGEABLE` · `CLEAN` · draft 유지). `deleteBranchOnMerge=false`는 그대로지만 base를 직접 옮겼으므로 무해하다 | ~~그대로 머지하면 **환영 알림톡이 prod에 안 나간다**~~ |
| 2 | ~~런북과 이 표가 반대를 말한다~~ → ✅ **닫혔다** (2026-08-22). 런북 §1이 **`SQL → env 교체(재배포 금지) → 머지 → 플래그`**로 재배열됐고 §1-2에 근거가 붙었다 | — |
| 3 | ~~🔴 **PR #66이 draft가 아니라 READY다**~~ → ✅ **닫혔다** (2026-09-10, `gh pr ready 66 --undo` 실행 + 확인. 현재 `isDraft=true`). `api/profile/setup/route.ts`에서 #53과 충돌하는 사실 자체는 그대로다 — **켜는 날이 끝난 뒤에 다시 꺼내 푼다** | ~~먼저 머지되면 켜는 날 당일에 손으로 충돌을 푼다~~ |
| 4 | ⚠️ **잔짐 — PR #40(`docs/redesign-claude-md`)이 열린 채 READY다.** 2535 리디자인 문서 PR로 켜는 날과 무관하다 | 위험은 낮다(사람이 눌러야 머지된다). 다만 런북 §1은 **env 교체 후 머지 전까지 재배포 금지 구간**을 두므로, 그 구간에 아무 PR도 머지되지 않도록 켜는 날 전에 정리(머지 또는 draft)하는 것을 권한다 |
> ✅ **같은 날 두 건이 더 닫혔다** (개발·운영 3차, 실제 머지 + `prelaunch`)
> - ~~머지된 조합을 아무도 빌드해본 적이 없다~~ → **처음으로 빌드했다.** lint 0 error · tsc 0 · **vitest 197 테스트 전부 통과** · build 성공. 표에 올린 당일 닫혔다
> - ~~"머지하면 충돌한다"(문서총괄 D2)~~ → **충돌 없다.** 임시 워크트리 실제 머지로 두 경로 다 통과, 최종 tree가 `b85e2f9`로 동일
>
> 🔴 **D2가 갈린 원인은 도구다 — 아무도 안 틀렸다.** `#53`은 **스택 PR**이고 진짜 merge base는 `a8e1c12`(`main`에 없다). #49가 그 뒤 추가한 커밋은 **`4878a56` 하나뿐**이라 #53은 #49의 대부분을 이미 담고 있다. base를 `main` 쪽으로 잡으면 두 PR이 *각자 같은 줄을 새로 추가한 것처럼* 보여 **add/add 충돌 3건**이 난다(관리자 재현 완료). **`merge-tree`로 스택 PR의 머지 가능성을 재지 않는다. 실제 머지가 유일한 심판이다.**
>
> **채택한 경로 = A(retarget).** 두 경로의 내용이 같으므로 기준은 되돌리는 단위뿐인데, 스택을 유지하면 `#49`+`#53`이 한 덩어리라 **환영 알림톡 하나가 문제여도 물어보기 크론까지 같이 내려간다.** A-3 격리 플래그를 만드는 이유와 같은 논리다.
>
> ✅ **게이트 2번도 닫혔다** — Vercel 공식 문서가 *"Changes to environment variables are not applied to previous deployments... You must redeploy your project"*라고 명시한다. 개발·운영이 🟨 추정으로 달았던 것이 참으로 확정됐다. **옛 경고문(*"env만 바꾸면 알림톡이 실패한다"*)은 틀린 문장이 아니라 「재배포하면」이라는 전제가 빠진 문장**이었고, 그래서 정확히 반대로 읽혔다. **위험한 것은 env 교체가 아니라 그 뒤의 재배포다.**
>
> ~~🔴 **남은 2건은 리모트 변경이라 단무지님 확인 대기 중이다**~~ → ✅ **2026-09-10 실행 완료.** `gh pr edit 53 --base main` · `gh pr ready 66 --undo` 둘 다 돌았고 `gh pr view`로 결과를 확인했다.
>
> 🔴 **이 칸이 남긴 교훈은 유효하다 — 결정과 실행은 다르다.** *"명령이 실행되기 전까지 닫힌 것이 아니다"*라고 적어둔 덕분에, 제품기획이 1번을 "닫혔다"로 보고한 뒤에도 **18일간 아무도 이것을 완료로 착각하지 않았다.** 실제로 실행된 것은 오늘이다. 리모트를 바꾸는 항목은 **명령 + 사후 조회 결과**를 같이 적는다.

> ⚠️ **`origin/main`은 `7d902f5`(PR #65까지)다** — 켜는 날 문서 여럿이 기준으로 박아둔 `0970fd0`보다 앞서 있다. **문서에 SHA를 박으면 낡는다.**

> ✅ **2026-08-22에 두 건이 닫혔다** (문서총괄 3차 점호 D1 + 제품기획 2차)
> - ~~`SOLAPI_TEMPLATE_WELCOME` env가 런북에 없다~~ → 런북 §1-1에 **필수 6번째 항목으로 승격**됨. 정확히는 *템플릿 ID는 있었고 **어느 env 키에 넣는지**가 없었다*
> - ~~`next_ui`·`library_enabled` 행이 prod에 없다~~ → 런북 §0에 **INSERT 대상 3개**로 반영됨(`discussion_meeting_enabled` 추가). 행이 없다는 사실 자체는 그대로 참이다(prod 키 12개, 2026-08-22 실측)
>
> ⚠️ **이 표가 하루 낡아 있었다.** 런북은 오늘 고쳐졌는데 이 표는 "4건"인 채였다 — 문서총괄이 D1로 잡았다. **이 표를 고치는 것은 관리자 소관**이라 런북을 고친 부서가 여기까지 손댈 수 없다. 런북이 바뀌면 이 표도 같은 세션에서 본다.
