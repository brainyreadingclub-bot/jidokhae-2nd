...
- `get_member_count()` — `profiles` 가입자 수만 반환(PII 0). SECURITY DEFINER + search_path='', anon/authenticated grant. 비로그인 로그인/웰컴 화면 회원수 라이브 카운트업용(anon RLS로 못 세므로 필요). 2535 리디자인에서 추가. 호출부(`auth/login/page`, `HomeContent` 웰컴 분기)는 `{ data }`만 구조분해 + `?? null` + `{count !== null &&}` 가드 → RPC 미적용 시 회원수 줄만 숨김(크래시 없음, forward-compatible). 마이그레이션: `migration-member-count.sql`
