...
| # | 남은 것 | 빠뜨리면 |
|:--:|---|---|
| 1 | **PR #53의 base가 `main`이 아니다**(`feat/book-ask-cron`) + `deleteBranchOnMerge=false`. ⚠️ **`#53`은 `#49` head를 포함하지 않는다** — 스택이 벌어져 있다(2026-08-22 `merge-base --is-ancestor` 실측) | 그대로 머지하면 **환영 알림톡이 prod에 안 나간다** |
| 2 | ~~런북과 이 표가 반대를 말한다~~ → ✅ **닫혔다** (2026-08-22). 런북 §1이 **`SQL → env 교체(재배포 금지) → 머지 → 플래그`**로 재배열됐고 §1-2에 근거가 붙었다 | — |
| 3 | 🔴 **PR #66이 draft가 아니라 READY다** (`feat/marketing-tracking`, 08-21 21:07 생성). `api/profile/setup/route.ts`에서 **#53과 실제 충돌**(관리자 재현). 어느 켜는 날 문서에도 없고 `META_*` env 4개가 신설된다 | 먼저 머지되면 **켜는 날 당일에 손으로 충돌을 푼다.** 순서 결정(#53 → #66)만으로는 안 막힌다 |
...
>
> 🔴 **남은 2건은 리모트 변경이라 단무지님 확인 대기 중이다** — `gh pr edit 53 --base main` · `gh pr ready 66 --undo`. **명령이 실행되기 전까지 닫힌 것이 아니다.** 제품기획이 1번을 "닫혔다"로 보고했으나 그건 **결정이 기록된 것**이지 리모트가 바뀐 것이 아니다 — 이 구분을 놓치면 켜는 날에 안 한 일을 했다고 믿는다.
