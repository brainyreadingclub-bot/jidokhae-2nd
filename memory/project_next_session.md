...
0. ~~알림톡 5종 실작동 점검~~(게이트 ①) → **2026-07-30 완료. 정상 확인.**
...
1. ~~[게이트 ②] 서재 콜드스타트 B안 Preview 검증~~ — 완료(위 0번). 아래는 결정 기록으로 보존.
   ...
   - B안이 감수한 대가: 물어보기가 한 번도 안 뜨는 회원(60일 내 정기모임 미참여)은 **서재를 영원히 못 본다**. 서재를 독립 기능으로 키우려면 이 결정을 다시 열어야 함.
2. **게이트 ②까지 통과 → `site_settings.library_enabled='on'` 토글**(배포 불필요) → 실제 전환율 측정 시작.
3. **운영자 알림 발송 이력 페이지** — 단무지님 요청(2026-07-30). 전문가 패널로 먼저 기획 예정. 정당성: 3~6월 123건 실패를 넉 달간 아무도 몰랐음. 최소 요건 후보 = 발송 실패의 능동적 노출, `sent`(접수)와 실제 전달의 구분, 종류·기간별 조회. 라우트 위치는 `adminMenu.ts`에 항목 추가(시스템 그룹 유력).
4. **알림톡 6번째 템플릿**(모임 다음날 오전 cron) — 물어보기의 **주 진입점**. confirmed 참여자에게 1회, answered/dismissed는 dedup skip, 딥링크로 물어보기. 재사용 기반: `getPendingAsk`/`verifyEligibleParticipation`(`asks.ts`), 알림톡 5종 패턴(`notification.ts`·`solapi.ts`), `cron/meeting-remind`(KST 19:00) 구조.
5. **알림톡 문구 개선** — 단무지님 요청(2026-07-30). ⚠️ 템플릿 수정은 **카카오 재심사** 대상이라 리드타임이 있고, 승인 전까지 기존 템플릿으로 발송됨. 리마인드는 `#{참가비}` 금액만 표시하고 결제 완료를 단정하지 않아 미입금자에게 보내도 어색하지 않으나, **미입금자에게 입금 안내를 덧붙이는 분기**가 있으면 낫다는 논의가 있었음(현재 단일 템플릿). 4번(6번째 템플릿 신설)과 묶어 심사 올리면 리드타임 절약.

### 콜드스타트 전수 결과 (2026-07-30, 참고용 보존)

문서 오류 2건 정정:
- **`LibraryIntroStrip`은 어디에서도 렌더되지 않는다** — import 0곳. `aa89215`에서 삽입됐다가 `2e1c41f`(1조각 UX 재설계)에서 제거된 듯. "소개 strip"·"strip 겹침" 상태는 **존재하지 않는다**. (죽은 코드 정리는 별건 — `LibrarySkeleton`은 PR #46에서 정리됨)
- **물어보기 strip 위치는 홈(/)이 아니라 마이페이지(/my)** — `AskStripSection`은 `/my`에만, `MeetingAskStripSection`은 모임 상세에만 렌더.

실제 `/my` 구성 순서: 내 정보(ProfileSection) → 물어보기 strip → 내 서재 → 내 신청. 서재는 독립 페이지가 아닌 `/my` 내 섹션.

실존 상태 4가지: ① 빈 서재 + 물어보기 없음 ② 빈 서재 + 물어보기 있음 ③ 책 있음 + 물어보기 없음 ④ 책 있음 + 물어보기 있음. **핵심 문제였던 ①을 B안이 제거.**
...
- ~~콜드스타트 시안 미착수~~ → **2026-07-31 B안 확정·구현·PR #46**. 남은 건 Preview 육안 검증뿐.
- ⚠️ **일반 회원(빈 서재) 첫 노출은 여전히 실계정 미검증** — 지금까지 등록 최다 계정으로만 확인했다. B안에서는 "아무것도 안 보이는 것"이 정답이라 검증 방법은 본인 서재를 비워서 확인하는 것.
